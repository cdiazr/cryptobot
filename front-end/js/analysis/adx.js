...
//                console.dir(row);                
                ...
                var iAdx = i - this._periodos;
                
                // mientra i < 28, acumular hasta llegar a los 13 periodos
                if ( iAdx < this._periodosADX ) { // hasta el 27 (suma de 14 periodos)
                    sumDX += row.DX;
                } 
                // legado al 28 calcular primer promedio ADX
                else if ( iAdx == this._periodosADX ) { 
                    sumDX += row.DX;
                    row.ADX = sumDX / this._periodosADX;
                } 
...
// -----------------------------------------------------------------------------
    ...
    _calcBase3 (row, rowAnt) {
        ...
        row.ADX = (rowAnt.ADX * (this._periodosADX - 1) + row.DX) / this._periodosADX;
        return row;
